...
export function updatePoolWaterVoid(poolGroup, spaGroup) {
  ...
  const updatePoolGeometryClip = (spa) => {
    ...
    if (spa) {
      ...
      const pad = 0.01;
      const margins = getSpaChannelMargins(spa, poolGroup);
      const expanded = getExpandedSpaWorldAABB(spa, margins, pad);
      if (!expanded) return;

      const minX = expanded.aabb.min.x;
      const maxX = expanded.aabb.max.x;
      const minY = expanded.aabb.min.y;
      const maxY = expanded.aabb.max.y;
      const minZ = sz - sh * 0.5 - pad;
      const maxZ = sz + sh * 0.5 + pad;
